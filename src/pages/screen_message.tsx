...
function MessagingUI() {
  ...
  const parsedUser = useMemo(
    () => (rawUserInfo ? JSON.parse(rawUserInfo) : null),
    [rawUserInfo]
  );

  const device_id = parsedUser?.user?.restaurants?.[0]?.device_id ?? null;
  const restaurant_id = parsedUser?.user?.restaurants?.[0]?.id ?? null;
  ...
  useEffect(() => {
    if (!device_id || !restaurant_id) return;
  ...
  const handleSubmit = (e: FormEvent<HTMLElement>) => {
    ...
    try {
      ws.current.send(JSON.stringify({ type: "message", message: inputValue }));
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 relative">
      {/* Header */}
      <header className="flex items-center gap-3 p-3 sm:p-4 bg-white border-b shadow-sm sticky top-0 z-20">
        <div className="relative">
          <div className="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center">
            <svg
              className="h-6 w-6 text-indigo-700"
      ...
      {/* Messages container */}
      <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-4 hide-scrollbar">
        <div className="flex flex-col space-y-4 pb-[100px]">
          {messages.map((msg) => (
            <div
              key={msg.id}
              className={`flex items-end ${
                msg.is_from_device ? "justify-end" : "justify-start"
              }`}
            >
              {/* Assistant avatar */}
              {!msg.is_from_device && (
                <div className="w-8 h-8 rounded-full bg-indigo-200 flex items-center justify-center mr-2 shadow-sm">
              ...
              {/* User avatar */}
              {msg.is_from_device && (
                <div className="w-8 h-8 rounded-full bg-blue-200 flex items-center justify-center ml-2 shadow-sm">
      ...
      {/* ✅ Input box */}
      <footer className="absolute lg:bottom-[6px] md:bottom-[2px] bottom-[1px] -left-2 w-full bg-white border-t border-gray-200 p-3 sm:p-4">
        <form
          onSubmit={handleSubmit}
        ...
        >
          <input
            ...
            className="flex-1 text-sm sm:text-base p-2 sm:p-3 border border-blue-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
          />
          <button
            type="submit"
            className="p-2 sm:p-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl transition flex items-center justify-center"
          >
            <Send size={20} />
          </button>
        </form>
